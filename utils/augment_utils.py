...
def random_size(image, target_size=None):
    ...
    if target_size is None:
        target_size = np.random.randint(*config.short_side_scale)
    if height < width:
...
def random_aspect(image):
    height, width, _ = np.shape(image)
    aspect_ratio = np.random.uniform(*config.aspect_ratio_scale)
    if height < width:
...
def random_crop(image):
    height, width, _ = np.shape(image)
    input_height, input_width, _ = config.input_shape
...
def random_hsv(image):
    random_h = np.random.uniform(*config.hue_delta)
    random_s = np.random.uniform(*config.saturation_scale)
    random_v = np.random.uniform(*config.brightness_scale)
...
def random_pca(image):
    alpha = np.random.normal(0, config.pca_std, size=(3,))
    offset = np.dot(config.eigvec * alpha, config.eigval)
    image = image + offset
...
def normalize(image):
    for i in range(3):
        image[..., i] = (image[..., i] - config.mean[i]) / config.std[i]
    return image
...
def test_10_crop(image):
    ...
    Then flip it, and do these again.
    """
    height, width, _ = np.shape(image)
    input_height, input_width, _ = config.input_shape
    ...
    images = [image[:input_height, :input_width, :], image[:input_height, -input_width:, :],
              image[-input_height:, :input_width, :], image[-input_height:, -input_width:, :],
              image[center_crop_y: center_crop_y + input_height, center_crop_x: center_crop_x + input_width, :],
              image_flip[:input_height, :input_width, :], image_flip[:input_height, -input_width:, :],
              image_flip[-input_height:, :input_width, :], image_flip[-input_height:, -input_width:, :],
              image_flip[center_crop_y: center_crop_y + input_height, center_crop_x: center_crop_x + input_width, :]]
